...
public class CartItemRepo {
    ...
    public void deleteCartItem(Integer productId,Integer cartId){
        entityManager.createNativeQuery("delete from itss.cart_item where product_id = ? and cart_id = ?")
    ...
    public void changeCartItem(Integer productId,Integer cartId,Integer quantity){
        entityManager.createNativeQuery("update itss.cart_item set quantity = ? where product_id = ? and cart_id = ?")
    ...
    public void decreaseCartItem(Integer productId,Integer cartId){
        entityManager.createNativeQuery("update itss.cart_item set quantity = quantity - 1 where product_id =? and cart_id = ?")
        .setParameter(1, productId)
        .setParameter(2, cartId)
        .executeUpdate();
    }
    @Transactional
    public void increaseCartItem(Integer cartId,Integer productId){
        entityManager.createNativeQuery("update itss.cart_item set quantity = quantity + 1 where cart_id = ? and product_id = ?")
        .setParameter(1, cartId)
        .setParameter(2, productId)
        .executeUpdate();
    }
